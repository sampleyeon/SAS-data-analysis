...
<details>
  ...
  - **결과:** Churn AUC 평균 v5 : 0.7924, LTV RMSE 평균 v5 : 1,385,450

</details>


<details>
  <summary><b>exp_v6: 하이브리드 앙상블 및 검증 전략 고도화 </b></summary>
  
  - **모델:** 
    - **CatBoost**: 두 모델의 예측값을 산술 평균하는 앙상블 기법을 통해 단일 모델이 가질 수 있는 오버피팅(Overfitting) 위험을 분산
    - **lightGBM**
  - **변경 사항:**
    - **금융 도메인 특화 피처 생성**: loan_asset_ratio(부채 대비 자산), debt_pressure(신용점수 대비 부채), net_cash_flow 등 금융 안정성을 나타내는 파생 변수를 추가하여 변별력을 높였습니다.
    - **학습 정밀도 최적화**: `learning_rate`를 기존보다 낮은 `0.005`로 설정하고, `num_leaves`를 `63`으로 늘려 데이터의 미세한 패턴을 더 깊게 학습하도록 유도.
    - **seed 변경**
  
  - **결과:** CV AUC 평균 v6 : 0.7974, CV RMSE 평균 v6 : 1,384,483, 대회 산식 최종 Score: 0.43173
